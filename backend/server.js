...
// ── 5. Routes (each path registered ONCE) ─────────────────────────────────
...
function generateMathQuestion(difficulty = 'easy') {
  let max, ops
  if (difficulty === 'easy')        { max = 10; ops = operations.slice(0, 2) }
  else if (difficulty === 'medium') { max = 12; ops = operations }
  else                              { max = 20; ops = operations }
...
app.get('/api/math', (req, res) => {
  res.json(generateMathQuestion(req.query.difficulty || 'easy'))
})
...
app.post('/api/validate/math', (req, res) => {
  ...
  res.json({ correct: parseInt(userAnswer) === parseInt(answer) })
})
